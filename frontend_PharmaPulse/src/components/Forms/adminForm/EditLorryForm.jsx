...
const EditLorryForm = ({ onUpdateLorry }) => {
  ...
  const handleSubmit = (e) => {
    ...
    if (!formData.lorryId || !formData.numberPlate || !formData.representativeId || !formData.capacity || !formData.dateOfAdded || !formData.status) {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col max-w-md mx-auto p-5 bg-[#e6eef3] rounded-lg shadow-md'>
      ...
      <div className='flex justify-between items-center mb-4'>
        <label htmlFor='status' className='text-[16px] text-gray-800 w-2/3'>Status:</label>
        <select
          id='status'
          name='status'
